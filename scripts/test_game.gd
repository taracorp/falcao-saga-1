...
var colors = [
	Color(0.0, 0.5, 0.3), Color(0.9, 0.7, 0.0), Color(0.3, 0.3, 0.4),
	Color(0.95, 0.95, 0.95), Color(0.85, 0.6, 0.1), Color(0.95, 0.85, 0.0),
]
var icons = ["⚽","🦅","🏟","👕","🏆","⭐"]
...
func _buy(cost, _type):
	if GameManager.spend_coins(cost):
		match _type:
			"moves": moves += 5; $Moves.text = "Moves: %d" % moves
...
func _draw_grid():
	for c in $GridLayer.get_children():
		c.queue_free()
	for r in ROWS:
		for cc in COLS:
			...
			rect.offset_right = x+CELL-2; rect.offset_bottom = y+CELL-2
...
func _input(event):
	...
	if event is InputEventMouseButton and event.pressed:
		...
		var c = int((pos.x - OX) / CELL)
		var r = int((pos.y - OY) / CELL)
