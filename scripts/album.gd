...
func _populate_season_selector() -> void:
	var selector = $MainVBox/SeasonSelector
...
func _on_season_selected(index: int) -> void:
	_current_season = $MainVBox/SeasonSelector.get_item_id(index)
	_refresh()
...
func _clear_grid() -> void:
	for child in $MainVBox/CardGrid.get_children():
		child.queue_free()
...
func _render_cards() -> void:
	...
	for card_id in range(start_id, end_id + 1):
		$MainVBox/CardGrid.add_child(_make_card(card_id))
...
func _make_card(card_id: int) -> Control:
	...
	if GameManager.has_card(card_id):
		...
		var rarity = int(data.get("foil", false)) + 1
...
func _update_progress() -> void:
	...
	for card_id in GameManager.cards:
		...
		if card_season == _current_season: owned += 1
	$MainVBox/ProgressLabel.text = "Progress: %d/%d cards (%.0f%%)" % [owned, total, float(owned)/float(total)*100]
